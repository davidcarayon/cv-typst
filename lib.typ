...
#let globe-link(url) = {
  ...
  align(right + horizon)[
    #fa-globe(fill: color-darkgray) #link(
      url,
      url,
    )
  ]
...
#let letter-heading(job-position: "", addressee: "") = {
  ...
  underline(evade: false, stroke: 0.5pt, offset: 0.3em)[
    #text(weight: "bold", size: 12pt)[Candidature pour le poste de #job-position]
  ]
